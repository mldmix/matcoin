...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " matcoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  matcoin-tx [options] <hex-tx> [commands]  Update hex-encoded matcoin transaction\n" +
            "or:     matcoin-tx [options] -create [commands]   Create hex-encoded matcoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded matcoin transaction
